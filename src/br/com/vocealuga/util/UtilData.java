...
public class UtilData {
	...
	/**
	 * Veja o método stringHora(GregorianCalendar)
	 */
	...
	/**
	 * Veja o método stringData(GregorianCalendar)
	 */
	...
	/**
	 * Veja o método stringDataCompleta(GregorianCalendar)
	 */
	...
	/**
	 * Instancia e retorna um GregorianCalendar, que já vem com o a data/hora
	 * atual
	 */
	 ...
	 */
	public static String somaDias(GregorianCalendar data, Integer dias) {

		if(Util.preenchido(data) && Util.preenchido(dias)){
	...
	/**
	 * Soma ou subtrai dias da data atual de acordo com o valor da variável
	 ...
	 * 
	 * @param dias
	 *            passado em módulo (passe um numero positivo!!!)
	...
	public static int getDia(String data) throws RuntimeException {
		...
		if (tokenizer.countTokens() != 3) {
			throw new RuntimeException("A data não está  no formato correto (dd/mm/aaaa)");
		}

		d = Integer.parseInt((String) tokenizer.nextElement());

		if (d <= 0 || d > 31) {
			throw new RuntimeException("O dia deve ser um número no intervalo entre 0 e 31");
		} else {
			dia = d;
		}
	...
	public static int getMes(String data) throws RuntimeException {
		...
		if (m <= 0 || m > 12) {
			throw new RuntimeException("O mês deve ser um número no intervalo entre 1 e 12");
		} else {
			mes = m;
		}
	...
	public static int getAno(String data) throws RuntimeException {
		...
		if (tokenizer.countTokens() != 3) {
			...
			// ano >100 indica que o ano tem 4 dígitos...
			// se tiver 3 estará errado...
			// nesse caso ano so pode ter 2 ou 4 dígitos
			if (ano < 100) { // 2 dígito - aqui o ano de 2 digitos e
				// transformado para 4
			...
			if (ano < 1980) {
				throw new RuntimeException("O ano não pode ser anterior a 1980");
			}
		}
	...
	/**
	 * <p>
	 * Recebe uma string que forma uma hora conta até o : e pega a primeira
	 * parte da string.
	 * </p>
	 * <p>
	 * Este método retorna um int.
	...
	public static int getHora(String horas) throws RuntimeException {
		...
		if (tokenizer.countTokens() != 2) {
			throw new RuntimeException("A hora não está  no formato correto (hh:mm)");
		}

		h = Integer.parseInt((String) tokenizer.nextElement());

		if (h > 23) {
			throw new RuntimeException("A hora não pode ser maior que 23");
		} else {
			hora = h;
		}
	...
	/**
	 * <p>
	 * Recebe uma string que forma uma hora conta até o : e pega a segunda parte
	 * da string.
	 * </p>
	 * <p>
	 * Este método retorna um int.
	 ...
	 * @param hora
	 *            - string a ser analisada .
	 * @return "minuto" ou entra em uma exceção
	 * @throws RuntimeException
	 *             - se o formato nao for reconhecido.
	 */
	public static int getMinuto(String horas) throws RuntimeException {
		...
		if (m > 59) {
			throw new RuntimeException("O minuto não pode ser maior que 59");
		} else {
			minuto = m;
		}
	...
	/**
	 * Converte um GregorianCalendar em uma string com o mes e o ano.
	 * 
	...
	public static String formatarHora(int hora, int minuto) {
		...
		/*
		 * a lógica abaixo serve para evitar que um horário como 09:02 apareça
		 * como 9:2
		 */
		if (hora < 10) {
			horario.append('0');
	...
	/**
	 * Chama o getCalendar(String) e retorna null caso não seja possível
	 * converter
	 */
	...
	/**
	 * Este método valida se a hora está  dentro do formato permitido para horas
	 * que variam de 00:00~24:00 (inclusive). É permitidos apenas o formato
	...
	/**
	 * Retorna a duração do apontamento
	 * 
	 * @param dataHoraInicio
	 * @param dataHoraFim
	 * @return duração do apontamento em horas
	 * @deprecated Esse método será apagado, utilizar
	...
	public static float calculaDuracaoHoras(GregorianCalendar dataHoraInicio, GregorianCalendar dataHoraFim) throws RuntimeException {
		...
		if (dataHoraFim == null || dataHoraInicio == null) {
			throw new RuntimeException("Os parâmetros \"dataHoraInicio\" e \"dataHoraFim\" não podem ser null");
		}
	...
	/**
	 * Calcula diferença em minutos. Retorna o módulo da diferença, pois a
	 ...
	 * @param dataHora2
	 *            segunda dataHora
	 * @return Módulo da diferença em minutos
	 */
	...
	/**
	 * Calcula diferença em minutos. Caso a dataHora1 seja posterior a
	 * dataHora2, o resultado será negativo.
	 * 
	 * @param dataHora1
	 *            primeira dataHora
	 * @param dataHora2
	 ...
	 * @throws RuntimeException
	 *             Caso um dos parâmetros seja null
	 */
	public static float calculaDiferencaMinutos(GregorianCalendar dataHora1, GregorianCalendar dataHora2) throws RuntimeException {
		...
		if (dataHora1 == null || dataHora2 == null) {
			throw new RuntimeException("Os parâmetros \"dataHora1\" e \"dataHora2\" não podem ser null");
		}
	...
	public static int converteStringHoraEmIntMinuto(String hora) throws Exception {
		...
		if (horas.length > 1) {
			...
			if (mm > 59) {
				throw new Exception("O número de minutos não pode ser maior que 59");
			}
	...
	public static Date gregorianCalendarToDate(GregorianCalendar gregorianCalendar) {
		...
		try {
			...
			log.error("Não foi possível converter um objeto GregorianCalendar para Date", e);
		}
	...
	public static GregorianCalendar getPrimeiroDiaSemana(GregorianCalendar dataAtual) {

		GregorianCalendar data = dataAtual;
		while(data.get(GregorianCalendar.DAY_OF_WEEK) != GregorianCalendar.SUNDAY){
			data.set(GregorianCalendar.DAY_OF_MONTH, data.get(GregorianCalendar.DAY_OF_MONTH)-1);
		}
	...
	/**
	 * Quando o gregorian calendar é construído apenas pelo dia, mês e ano a
	 * hora fica sendo meia-noite. Na pesquisa isso é inadequado porque tudo o
	 * que aconteceu após a meia-noite é desconsiderado. O método tenta resolver
	...
	public static List<String> listarMesesAno(GregorianCalendar dataInicio, GregorianCalendar dataFim) {
		...
		while ((ano < anoFim) || (ano == anoFim && mes <= mesFim)) {
			...
			case 11:
				...
				break;
			default:
				throw new RuntimeException("O mês não foi definido corretamente");
			}
	...
	/**
	 * Verifica se o mês e o ano das duas datas são iguais.
	 * 
	 * @param gregorianCalendar1
	 * @param gregorianCalendar2
	 * @return true se o mês e o ano forem iguais, false caso contrário.
	 */
	...
	/**
	 * Verifica se o dia, mês e o ano das duas datas são iguais.
	 * 
	 * @param gregorianCalendar1
	 * @param gregorianCalendar2
	 * @return true se o dia,mês e o ano forem iguais, false caso contrário.
	 */
	...
	/**
	 * Método que converte o tempo de hh:mm:ss ou hh:mm para segundos
	 * 
	 * @param HHMMSS
	...
	/**
	 * Retorna true se a String está  no formato hh:mm:ss, ou false caso
	 * contrário.
	 */
	...
	/**
	 * Método que converte de segundos para a mêscara
	 * 
	 * @param segundos
	 *            o tempo em segundos
	 * @return String a mêscara
	 */
	...
	/**
	 * Método que converte de segundos para a mêscara considerando segundos
	 * negativo
	 * 
	 * @param segundos
	 *            o tempo em segundos
	...
	/**
	 * Calcula a diferença em dias entre as duas datas, ou seja ignorando as
	 * horas. Ex: 15/06 as 15:00 até 16/06 as 2:00 a diferença é 1 dia
	 * 
	 * @param dataInicio
	 * @param dataFim
	 */
	 ...
	 * 
	 * Ex: 2 dias é igual a 48:00:00 horas
	 * 
	 * Ex: 2,5 dias é igual a 60:00:00 horas
	 */
	 ...
	 * 
	 * Ex: 216000 segundos é igual a 2 dias e meio
	 */
	...
	/**
	 * Obtém o dia da semana da data passada como parâmetro
	 */
	public static String getDiaSemana(GregorianCalendar data) {
		...
		dias.put(GregorianCalendar.SATURDAY, "Sáb");
